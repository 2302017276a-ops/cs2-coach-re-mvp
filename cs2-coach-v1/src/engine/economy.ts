...
export function applyBuy(team: TeamState, mode: 'eco' | 'semi' | 'force' | 'full') {
  const cost = { eco: 500, semi: 1800, force: 3200, full: 4200 }[mode];
  team.money = Math.max(0, team.money - cost);
}

export function resolveRoundMoney(
  winner: TeamState,
  loser: TeamState,
  cause: 'elim' | 'bomb' | 'defuse',
  kills: { team: 'winner' | 'loser'; weapon: keyof typeof killReward }[],
  bombPlantedByLoser = false,
) {
  ...
  for (const k of kills) {
    (k.team === 'winner' ? winner : loser).money += killReward[k.weapon];
  }
